...
export async function obtenirRessource(nom: NomRessource, id: string) {
  if (nom === "ouvrages") {
    ...
    const detail = await pool.query(`SELECT o.*,to_jsonb(m.*) matiere,to_jsonb(n.*) niveau_scolaire,
      ...
      JOIN ${schema}.niveaux_scolaires n ON n.id=o.niveau_scolaire_id
      LEFT JOIN ${schema}.exemplaires e ON e.ouvrage_id=o.id WHERE o.id=$1 GROUP BY o.id,m.id,n.id`, [id])
