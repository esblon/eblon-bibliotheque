...
export async function listerEmprunts(p:Pagination,params:URLSearchParams){const vals:unknown[]=[];const cond:string[]=[];for(const f of ["statut","emprunteur_id","exemplaire_id"]){const v=params.get(f);if(v){vals.push(v);cond.push(`${f}=$${vals.length}`)}}for(const f of ["date_emprunt","date_echeance"]){const v=params.get(f);if(v){const date=new Date(v);if(Number.isNaN(date.valueOf()))throw new ErreurApi("DONNEES_INVALIDES",`Filtre ${f} invalide`,400);vals.push(date);cond.push(`${f}::date=$${vals.length}::date`)}}if(params.get("en_retard")==="true")cond.push(`statut IN ('ACTIF','EN_RETARD') AND date_echeance<current_timestamp`);const w=cond.length?`WHERE ${cond.join(" AND ")}`:"";const t=(await pool.query<{total:number}>(`SELECT count(*)::int total FROM ${schema()}.emprunts ${w}`,vals)).rows[0]?.total??0;vals.push(p.limite,(p.page-1)*p.limite);const r=await pool.query(`SELECT * FROM ${schema()}.emprunts ${w} ORDER BY date_emprunt ${p.ordre==="desc"?"DESC":"ASC"} LIMIT $${vals.length-1} OFFSET $${vals.length}`,vals);return{donnees:r.rows,total:t}}
