...
export async function listerReferentielsInscription(){
 const [classes,etablissements]=await Promise.all([
  pool.query<ClasseInscription&{niveau_nom:string}>(`SELECT c.id,c.niveau_scolaire_id,c.nom,n.nom niveau_nom FROM ${schema()}.classes_scolaires c JOIN ${schema()}.niveaux_scolaires n ON n.id=c.niveau_scolaire_id WHERE c.est_active AND n.est_actif ORDER BY n.code,c.ordre,c.nom`),
  pool.query<EtablissementInscription>(`SELECT id,nom FROM ${schema()}.etablissements WHERE est_actif ORDER BY nom`),
 ])
 const niveaux=new Map<string,NiveauInscription>()
 for(const classe of classes.rows){if(!niveaux.has(classe.niveau_scolaire_id))niveaux.set(classe.niveau_scolaire_id,{id:classe.niveau_scolaire_id,nom:classe.niveau_nom,classes:[]});niveaux.get(classe.niveau_scolaire_id)?.classes.push({id:classe.id,niveau_scolaire_id:classe.niveau_scolaire_id,nom:classe.nom})}
