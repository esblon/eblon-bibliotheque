...
export const validationsRessources = {
  ...
  ouvrages: z.object({ titre: texte(300), sous_titre: optionnel(300), isbn: optionnel(30), editeur: optionnel(150), edition: optionnel(100), annee_publication: z.number().int().min(1000).max(9999).nullable().optional(), description: optionnel(5000), matiere_id: uuid, niveau_scolaire_id: uuid, cle_image_couverture: optionnel(500), est_actif: z.boolean().optional() }),
  ...
  emprunteurs: z.object({ numero_emprunteur: texte(100).transform((v) => v.toUpperCase()), prenom: texte(150), nom: texte(150), email: z.string().email().max(255).nullable().optional(), telephone: optionnel(50), niveau_scolaire_id: uuid.nullable().optional(), classe: optionnel(100), etablissement: optionnel(255), statut: z.enum(["ACTIF","SUSPENDU","ARCHIVE"]).optional() }),
  agents: z.object({ identifiant_auth_externe: optionnel(255), prenom: texte(150), nom: texte(150), email: z.string().email().max(255), role: z.enum(["ADMIN","ENSEIGNANT","BIBLIOTHECAIRE","LECTEUR"]), statut: z.enum(["ACTIF","DESACTIVE"]).optional() }),
}
