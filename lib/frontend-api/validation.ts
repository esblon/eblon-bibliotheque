...
export const schemasFormulaires = {
  ...
  ouvrages: z.object({ titre:texte(300), sous_titre:optionnel(300), isbn:optionnel(30), editeur:optionnel(150), edition:optionnel(100), annee_publication:z.union([z.coerce.number().int().min(1000).max(9999),z.literal("")]).transform(v=>v===""?null:v), description:optionnel(5000), matiere_id:z.string().uuid(), niveau_scolaire_id:z.union([z.string().uuid(),z.literal("")]).transform(v=>v||null), est_actif:z.boolean() }),
  ...
  agents: z.object({ identifiant_auth_externe:optionnel(255), prenom:texte(150), nom:texte(150), email:z.string().email("Email invalide"), role:z.enum(["ADMIN","ENSEIGNANT","BIBLIOTHECAIRE","LECTEUR"]), statut:z.enum(["ACTIF","DESACTIVE"]) }),
}
