...
export async function enregistrerRessource(ressource:RessourceEditable,id:string|null,valeurs:unknown):Promise<EtatAction>{const resultat=schemasFormulaires[ressource].safeParse(valeurs);if(!resultat.success)return{succes:false,message:"Corrigez les champs indiqués.",erreurs:resultat.error.flatten().fieldErrors as Record<string,string[]>};try{if(id)await apiFrontend.modifier(ressource,id,resultat.data);else{const cree=await apiFrontend.creer<Agent>(ressource,resultat.data);if(ressource==="agents"&&emailEnabled())await inviterAgent(cree.donnees)}revalidatePath(`/${ressource}`);revalidatePath("/");return{succes:true,message:id?"Modification enregistrée.":ressource==="agents"?(emailEnabled()?"Agent créé et invitation envoyée.":"Agent créé. L’envoi d’e-mails est désactivé."):"Création enregistrée."}}catch(error){return{succes:false,message:messageErreur(error)}}}
